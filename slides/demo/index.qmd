...
::: {.large}
This is a [Quarto](https://quarto.org/) [revealjs](https://revealjs.com) presentation. This demo showcases the usage and capabilities of this system. Revealjs in Quarto is documented [here](https://quarto.org/docs/presentations/revealjs/).
:::

Press `q` to enable pointer and press `q` again to hide it
...
Source code blocks can be folded to only show the output.
...
Quarto extension [`quarto-collapse-output`](https://github.com/mcanouil/quarto-collapse-output) is used to enable folding of code output.
...
Quarto extension [leaflet](https://github.com/royfrancis/quarto-leaflet) provides support for [leafletjs](http://leafletjs.com/) maps. Add map information to yaml matter or `_metadata.yml` and use the shortcode `{{< leaflet mapname >}}`.
